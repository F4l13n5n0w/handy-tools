...
echo "[+] install handy tools for kali 2026.1 VM"


sudo apt-get update 
sudo apt-get full-upgrade -y

sudo apt-get install -y dirsearch pacu cloudbrute flameshot
sudo apt-get install -y testssl.sh seclists gnome-terminal subfinder gron jq
...
cd /opt/
wget https://github.com/epi052/feroxbuster/releases/download/v2.13.1/feroxbuster_amd64.deb.zip -O ./feroxbuster.zip
unzip ./feroxbuster.zip
dpkg -i feroxbuster_2.13.1-1_amd64.deb
rm -rf ./feroxbuster*
...
cd /opt/
wget https://download.sublimetext.com/sublime-text_build-4200_amd64.deb -O sublime4.deb
dpkg -i ./sublime4.deb
rm -rf ./sublime4.deb
...
# Install neo4j and bloodhound CE GUI
sudo apt-get install -y bloodhound-cd-python bloodhound
...
# Change hostname 
echo "127.0.0.1   spiderlab-tester" >> /etc/hosts
hostnamectl set-hostname spiderlab-tester
